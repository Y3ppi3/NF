 ...
 */
async function tryEndpoints(endpoints, method = 'get', data, fallbackData) {
...
export function setOfflineMode(enable) {
    offlineMode = enable;
...
export async function checkApiHealth() {
    try {
        const result = await tryEndpoints(API_ENDPOINTS.health, 'get', undefined, { status: 'offline', message: 'Автономный режим активен' });
...
export async function getCategories() {
    return tryEndpoints(API_ENDPOINTS.categories, 'get', undefined, FALLBACK_CATEGORIES);
}
/**
 * Функции для работы с товарами
 */
export async function getProducts() {
    return tryEndpoints(API_ENDPOINTS.products, 'get', undefined, FALLBACK_PRODUCTS);
}
export async function getProductsByCategory(categoryId) {
    const endpoints = API_ENDPOINTS.products.map(endpoint => `${endpoint}/category/${categoryId}`);
    // Для автономного режима фильтруем локальные данные
    const offlineData = FALLBACK_PRODUCTS.filter(p => p.category_id.toString() === categoryId.toString());
    return tryEndpoints(endpoints, 'get', undefined, offlineData);
}
export async function getProductById(productId) {
    const endpoints = API_ENDPOINTS.products.map(endpoint => `${endpoint}/${productId}`);
    // Для автономного режима находим товар в локальных данных
    const offlineProduct = FALLBACK_PRODUCTS.find(p => p.id.toString() === productId.toString());
    return tryEndpoints(endpoints, 'get', undefined, offlineProduct);
}
/**
...
export async function getCart() {
    ...
    return tryEndpoints(API_ENDPOINTS.cart, 'get', undefined, emptyCart);
}
export async function addToCart(data) {
    return tryEndpoints(API_ENDPOINTS.cart, 'post', data, { success: true, message: 'Товар добавлен в корзину (автономный режим)' });
}
export async function updateCartItem(itemId, data) {
    const endpoints = API_ENDPOINTS.cart.map(endpoint => `${endpoint}/${itemId}`);
    return tryEndpoints(endpoints, 'put', data, { success: true, message: 'Количество товара изменено (автономный режим)' });
}
export async function removeFromCart(itemId) {
    const endpoints = API_ENDPOINTS.cart.map(endpoint => `${endpoint}/${itemId}`);
    return tryEndpoints(endpoints, 'delete', undefined, { success: true, message: 'Товар удален из корзины (автономный режим)' });
}
/**
...
export async function login(credentials) {
    try {
        ...
        const response = await tryEndpoints(API_ENDPOINTS.auth, 'post', credentials);
...
export async function register(userData) {
    ...
    try {
        const response = await tryEndpoints(API_ENDPOINTS.register, 'post', userData);
        // Сохраняем данные авторизации
        Storage.setToken(response.access_token);
        Storage.setTokenType(response.token_type);
        Storage.setUserId(response.user.id.toString());
...
export async function createOrder(orderData) {
    ...
    const offlineOrder = {
        ...
        status: 'pending',
        total_amount: 0,
        user_id: Storage.getUserId() ? parseInt(Storage.getUserId()) : 1,
        created_at: new Date().toISOString(),
        updated_at: new Date().toISOString(),
        items: [],
        ...orderData
    };
    return tryEndpoints(API_ENDPOINTS.orders, 'post', orderData, offlineOrder);
}
export async function getOrders() {
    // В автономном режиме возвращаем пустой список заказов
    return tryEndpoints(API_ENDPOINTS.orders, 'get', undefined, []);
}
export async function getOrderById(orderId) {
    ...
    return tryEndpoints(endpoints, 'get', undefined, offlineOrder);
}
